...
// Constants :
#define kSureButtonItemTitle NSLocalizedString(@"I18N_COMMON_DETERMINE", nil)
#define kCancelButtonItemTitle NSLocalizedString(@"I18N_COMMON_CANCLE", nil)
...
static CGFloat const kHooDatePickerButtonHeaderWidth = 74.0f;
...
- (void)buildSelectorLabelsDates {
    ...
    [dateFormatter setDateFormat:[NSString stringWithFormat:@"MMMdd%@ EEE", NSLocalizedString(@"", nil)]];
...
//      \u5e74  年
        [years addObject:[NSString stringWithFormat:@"%ld%@", (long)i, NSLocalizedString(@"", nil)]];
    }
...
- (NSMutableArray*)getDaysInMonth:(NSDate*)date {
    ...
    for (int i = 1; i <= daysRange.length; i++) {
        ...
        [days addObject:[NSString stringWithFormat:@"%d%@", i, NSLocalizedString(@"", nil)]];
    }
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    if (self.datePickerMode != HooDatePickerModeDateAndTime && self.datePickerMode != HooDatePickerModeTime) {
        if ([selectedDate compare:self.minimumDate] == NSOrderedAscending) {
            ...
            NSLog(@"%@", NSLocalizedString(@"I18N_COMMON_APP_MINIMUM_DATE_PICK_TIP", nil));
        }

        if ([selectedDate compare:self.maximumDate] == NSOrderedDescending) {
            [self setDate:self.maximumDate animated:YES];
            NSLog(@"%@", NSLocalizedString(@"I18N_COMMON_APP_MAXIMUM_DATE_PICK_TIP", nil));
        }
    }
...
- (NSMutableArray*)hours {
    if (!_hours) {
        ...
        for (int i = 0; i < 24; i++) {
            if (i < 10) {
                [hours addObject:[NSString stringWithFormat:@"0%d%@", i, NSLocalizedString(@"", nil)]];
            } else {
                [hours addObject:[NSString stringWithFormat:@"%d%@", i, NSLocalizedString(@"", nil)]];
            }
        }
        _hours = hours;
    }
...
- (NSMutableArray*)minutes {
    if (!_minutes) {
        ...
        for (int i = 0; i < 6; i++) {
            if (!i) {
                [minutes addObject:[NSString stringWithFormat:@"%d0%@", i, NSLocalizedString(@"", nil)]];
            } else {
                [minutes addObject:[NSString stringWithFormat:@"%d%@", i * 10, NSLocalizedString(@"", nil)]];
            }
